...
## ⚠️ CRITICAL: Correct Configuration File Location

**Claude Code uses `.mcp.json` in the PROJECT ROOT, NOT a global settings file!**

- **✅ CORRECT**: `.mcp.json` in your project root directory
- **❌ WRONG**: `~/.config/claude-code/mcp-settings.json` (does NOT work!)
- **Alternative**: Use Claude CLI: `claude mcp add --transport stdio cognitive-memory -- /path/to/server`
...
```json
{
  "mcpServers": {
    "cognitive-memory": {
      ...
      "args": [
        "-c",
        "DATABASE_URL='postgresql://postgres:postgres@localhost:54322/postgres' ANTHROPIC_API_KEY='sk-ant-api03-YOUR_ANTHROPIC_API_KEY' OPENAI_API_KEY='sk-placeholder' ENVIRONMENT='production' /path/to/python -m mcp_server"
      ]
    }
  }
}
...
- **7 Tools** should be available:
  ...
  - `get_golden_test_results` - Golden test results (stub)
...
1. Check MCP server logs should show: "Registered 7 tools" and "Registered 5 resources"
...
*Project: Cognitive Memory System v3.1.0-Hybrid*
